from starkit import __path__ as STARKIT_PATH
...
STARKIT_PATH = STARKIT_PATH[0]
DEFAULT_CONFIG_PATH = os.path.join(STARKIT_PATH, 'data', 'default_starkit_config.yml')

DEFAULT_DATA_DIR = os.path.join(os.path.expanduser('~'), 'Downloads', 'starkit')
logger = logging.getLogger(__name__)

def get_configuration():

    config_fpath  = os.path.join(get_config_dir(), 'starkit_config.yml')
...
def get_data_dir():
    ...
    if data_dir is None:
        config_fpath = os.path.join(get_config_dir(), 'starkit_config.yml')
        logging.critical('\n{line_stars}\n\nSTARKIT DATA DIRECTORY not specified in {config_file}:\n\n'
